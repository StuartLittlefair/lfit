...
LD_FLAGS = $(LDFLAGS) -L/usr/local/lib -L$(TRM_SOFTWARE)/lib -L/usr/X11/lib
CPP_FLAGS = $(CPPFLAGS) -I/usr/local/include -I$(TRM_SOFTWARE)/include -I/usr/X11/include -I./include
...
lfit: src/lfit.cc $(SOURCES) $(INCLUDES)
	$(CC) $(CFLAGS) src/lfit.cc $(SOURCES) $(LD_FLAGS) $(CPP_FLAGS) $(TRM_LIBS) $(PGPLOT_LIBS) $(EXTRA_LIBS) -o lfit
	
lfit_fake: src/lfit_fake.cc $(SOURCES) $(INCLUDES)
	$(CC) $(CFLAGS) src/lfit_fake.cc $(SOURCES) $(LD_FLAGS) $(CPP_FLAGS) $(TRM_LIBS) $(PGPLOT_LIBS) $(EXTRA_LIBS) -o lfit_fake