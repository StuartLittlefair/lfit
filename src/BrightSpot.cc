...
void LFIT::BrightSpot::setup_grid(const double &incl)
{
    ...
    // find the length of bright spot in scale lengths

    // step1: location of brightest point (scaled by scale length) is BMAX
    ...
    for (int i = 0; i < nspot; i++)
    {
        ...
        if (Roche::ingress_egress(q, Roche::SECONDARY, 1.0, 1.0, incl, 1.0e-8, posn, ingress, egress))
        {
...
double LFIT::BrightSpot::calcFlux(const double &q, const double &phi, const double &incl)
{
    ...
    if (!this->complex)
    {
        return LFIT::BrightSpot::simpleFlux(q, phi, incl);
    }
    ...
    if (this->normalisation < 0.0)
    {
        for (int i = 0; i < this->spot.size(); i++)
        {
            ...
            if (i < nspot)
            {
                // tilted strip
                if (mumax > 0.)
                    maxflux += mumax * this->spot[i].flux;
            }
            else
            {
                // parallel strip
                if (mumax > 0.)
                    maxflux += maxProj * this->spot[i].flux;
            }
        }
    ...
    for (int i = 0; i < this->spot.size(); i++)
    {
        ...
        {
            // tilted strip
            if (muact > 0. && this->spot[i].visible(phi))
                bflux += muact * this->spot[i].flux;
        }
        else
        {
            // parallel strip
            if (muact > 0. && this->spot[i].visible(phi))
                bflux += maxProj * this->spot[i].flux;
        }
    }
